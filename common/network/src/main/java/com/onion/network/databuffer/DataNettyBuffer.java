...
public final class DataNettyBuffer implements DataBuffer {
    ...
    public ByteBuffer getReadOnlyByteBuffer() {
        ByteBuffer mbuffer = this.buffer.asReadOnlyBuffer();
        buffer.position(0);
        return buffer;
    }
     ...
     */
    public void release() {
        Preconditions.checkState(nettyBuf != null);
        Preconditions.checkState(nettyBuf.refCnt() == 1,
                "Reference count of the netty buffer is %s (1 expected).", nettyBuf.refCnt());
        Preconditions.checkState(nettyBuf.release() == true, "Release Netty ByteBuf failed.");
    }
}
